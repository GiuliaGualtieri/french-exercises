...
class FrenchApp(customtkinter.CTk):
    def __init__(self):
        ...
        self.title_label = customtkinter.CTkLabel(
            ...
            text="Let's practice in French!",
            font=customtkinter.CTkFont(
                family="Calibri", size=30, weight="bold", slant="roman"
            ),
        )
        self.title_label.grid(
            row=0, column=1, columnspan=2, padx=(20, 20), pady=(20, 20)
        )
        # text question
        self.textbox = customtkinter.CTkTextbox(
            master=self.frame_title_text, font=self.my_font
        )
        self.textbox.grid(
            row=1, column=1, columnspan=2, padx=(20, 20), pady=(10, 20), sticky="nsew"
        )
        ...
        self.combobox = customtkinter.CTkComboBox(
            master=self, values=self.possible_answers(), font=self.my_font
        )
        ...
        self.button = customtkinter.CTkButton(
            master=self,
            command=self.insert_answer_callback,
            text="Insert Answer",
            font=self.my_font,
        )
        ...
        self.button = customtkinter.CTkButton(
            master=self,
            command=self.next_question_callback,
            text="Next Question",
            font=self.my_font,
        )
        ...
        self.appearance_mode_label = customtkinter.CTkLabel(
            master=self, text="Appearance Mode:", anchor="w", font=self.my_font
        )
        ...
        self.scaling_label = customtkinter.CTkLabel(
            master=self, text="UI Scaling:", anchor="w", font=self.my_font
        )
    ...
    def next_question_callback(self):
        ...
        if self.counter < 20:
            ...
            self.textbox.insert(
                "insert", f"YOUR FINAL SCORE IS {str(self.right)} over {self.counter}"
            )
            self.textbox.configure(state="disabled")
